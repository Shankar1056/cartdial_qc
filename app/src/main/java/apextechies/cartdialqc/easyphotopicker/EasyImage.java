package apextechies.cartdialqc.easyphotopicker;
...
public class EasyImage implements Constants {
    ...
    private static void onPictureReturnedFromDocuments(Intent data, Activity activity, @NonNull Callbacks callbacks) {
        try {
            ...
            callbacks.onImagesPicked(EasyImageFiles.singleFileList(photoFile), ImageSource.DOCUMENTS, restoreType(activity));

            if (configuration(activity).shouldCopyPickedImagesToPublicGalleryAppFolder()) {
                EasyImageFiles.copyFilesInSeparateThread(activity, EasyImageFiles.singleFileList(photoFile));
            }
        } catch (Exception e) {
            e.printStackTrace();
